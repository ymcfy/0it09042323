...
public class FileUpLoadController : MonoBehaviour
{
    ...
    {
        sqlCon.Open();
        string strJudgeSql = "select * from [SqlTest].[dbo].[Model] where modelName = '" + modelName + "'";
        //检索保存数据
        SqlDataAdapter sqlAdapter = new SqlDataAdapter(strJudgeSql, sqlCon);
        ...
        {
            //模型目标存储路径
            string path = Application.dataPath + "//Resources//ModelBase//model/" + modelName + modelType;
            string modelPath = modelName + modelType;

            //模型图片存储路径
            string imgDestPath = Application.dataPath + "//Resources//ModelBase//modelImage/" + modelName + ImgType;
            string imPath = modelName + ImgType;
            ...
            //定义数据库添加操作语句 ‘“++”’
            string strSql = "insert into Model(modelName,modelPath，modelImage) values('" + modelName+"','" + modelPath + "','"  + imPath + "')";
